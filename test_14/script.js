...
window.EXAM_NAME = 'UCAT Situational Judgement Mock 3 Test';
...
async function doSubmit() {
  ...
  // Calculate scaled score for the SJT subtest
  const subtests = ['Situational Judgement'];
  const topicStats = {};
  ...
  const result = {
    ...
    topicStats,
    examName: 'UCAT Situational Judgement Mock 3 Test',
    topicNumber: 5,
    details, answers,
...
function renderResults(result, saveRes) {
  ...
  // No passages used in SJT data, but keep same review grouping structure
  let reviewHTML = '';
  if (wrongDetails.length > 0) {
    wrongDetails.forEach(q => {
  ...
  if (!result.topicStats) {
    topicStats[subtests[0]] = { correct: 0, total: 0, scaled: 300 };
    details.forEach(d => {
      const topic = d.topic;
      if (topicStats[topic]) {
        topicStats[topic].total++;
    ...
    subtests.forEach(topic => {
      const stats = topicStats[topic];
      if (stats.total > 0) stats.scaled = 300 + Math.round((stats.correct / stats.total) * 600);
    });
  }
  ...
  wrap.innerHTML = `
          ...
          return `
            <div style="background:var(--surface2); border:1px solid var(--border); border-radius:10px; padding:16px; display:flex; flex-direction:column; gap:8px;">
              <div style="display:flex; justify-content:space-between; align-items:center;">
                <span style="font-weight:600; font-size:0.9rem; color:var(--text);">${topic === 'Decision Making' ? '🧩 ' : topic === 'Quantitative Reasoning' ? '🔢 ' : topic === 'Situational Judgement' ? '⚖️ ' : '📖 '}${topic}</span>
    ...
    ${wrongDetails.length > 0
      ? `<div class="inc-review"><h3>⚠️ Review: ${wrong} Wrong &nbsp;+&nbsp; ${unattempted} Skipped</h3>${reviewHTML}</div>`
      : `<div class="inc-review" style="text-align:center;padding:32px"><div style="font-size:3rem;margin-bottom:8px">🎉</div><h3 style="color:var(--green);border:none;padding:0">Perfect Score!</h3></div>`}
  ...
  new Chart($('pieChart'), {
    ...
    options: {
      plugins: { legend: { position: 'bottom', labels: { font: { family: 'IBM Plex Sans' }, color: '#8b83a3' } } },
      cutout: '68%'
    }
  });
  ...
  const bands = subtests.map(topic => {
    const stats = topicStats[topic];
    return { label: topic, score: stats.scaled, total: 900 };
  });
  ...
  new Chart($('barChart'), {
    ...
    data: {
      ...
      datasets: [{
        label: 'Scaled Score',
        data: bScores,
        backgroundColor: bColors.map(c => c + '33'),
        borderColor: bColors,
        borderWidth: 2,
        borderRadius: 6
      }]
    ...
    options: {
      ...
      scales: {
        y: {
          min: 300,
          max: 900,
          ticks: { stepSize: 100, font: { family: 'IBM Plex Mono' }, color: '#8b83a3' },
