window.EXAM_NAME = 'Decision Making Mock Test';
// ══════════════════════════════════════════════
...
async function doSubmit() {
  ...
  const ucatReport = typeof UCATScoring !== 'undefined'
    ? UCATScoring.generateUCATReport({
        ...
        examName: window.EXAM_NAME || 'Decision Making Mock Test',
        questions: QUESTIONS,
...
function renderResults(result, saveRes) {
  ...
  if (isSJT) {
    ...
    const secName = window.EXAM_NAME || 'Decision Making Mock Test';
  ...
  wrap.innerHTML = `
    <div class="res-hero">
      <div style="font-size:0.8rem;opacity:.55;text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px">${window.EXAM_NAME || 'Decision Making Mock Test'}</div>
      ${scoreHeroHTML}
