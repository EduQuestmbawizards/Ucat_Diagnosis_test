...
function initTest() {
  ...
  buildPalette();
  renderQ();
  startTimer(102 * 60); // 102 minutes for the 4 subtests
...
async function doSubmit() {
  ...
  // Calculate scaled scores for the 4 subtests
  const subtests = [
    'Verbal Reasoning',
    'Decision Making',
  ...
  const result = {
    ...
    topicStats,
    examName: 'UCAT Full Mock Test (No AR)',
    topicNumber: 6,
    details, answers,
...
function renderResults(result, saveRes) {
  ...
  wrap.innerHTML = `
    <div class="res-hero">
      <div style="font-size:0.8rem;opacity:.55;text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px">UCAT Mock Test 6 (No AR)</div>
      <div class="sat-score" style="color:${scoreColor}">${scaled} <span style="font-size:2rem; font-family:var(--font); color:var(--muted); font-weight:normal;">/ 3600</span></div>
    ...
    <div class="chart-card" style="margin-bottom:20px;">
      ...
      <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap:16px;">
        ${subtests.map(topic => {
          ...
          else if (topic === 'Situational Judgement') subtestIcon = '⚖️';
