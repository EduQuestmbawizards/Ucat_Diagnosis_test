window.EXAM_NAME = 'UCAT Verbal Reasoning Mock 2';
// ══════════════════════════════════════════════
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $('sectionLabel').textContent = `Verbal Reasoning · ${totQs} Questions`;
});
...
function initTest() {
  ...
  buildPalette();
  renderQ();
  startTimer(120 * 60); // 120 minutes for 250 questions
...
async function doSubmit() {
  ...
  const ucatReport = typeof UCATScoring !== 'undefined'
    ? UCATScoring.generateUCATReport({
        ...
        examName: window.EXAM_NAME || 'UCAT Verbal Reasoning Mock 2',
        questions: QUESTIONS,
...
function renderResults(result, saveRes) {
  ...
  if (isSJT) {
    ...
    const secName = window.EXAM_NAME || 'UCAT Verbal Reasoning Mock 2';
  ...
  wrap.innerHTML = `
    <div class="res-hero">
      <div style="font-size:0.8rem;opacity:.55;text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px">${window.EXAM_NAME || 'UCAT Verbal Reasoning Mock 2'}</div>
      ${scoreHeroHTML}
