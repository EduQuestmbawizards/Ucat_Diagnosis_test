window.EXAM_NAME = 'UCAT Decision Making Mock 2';
// ══════════════════════════════════════════════
...
// Update section header to show correct total count
document.addEventListener('DOMContentLoaded', () => {
  const totQs = QUESTIONS.length;
  $('sectionLabel').textContent = `Decision Making · ${totQs} Questions`;
});
...
function initTest() {
  ...
  buildPalette();
  renderQ();
  startTimer(100 * 60); // 100 minutes for 118 questions
...
function renderQ() {
  ...
  $('qText').innerHTML = q.text; // Support image tags inside question text
...
async function doSubmit() {
  ...
  const ucatReport = typeof UCATScoring !== 'undefined'
    ? UCATScoring.generateUCATReport({
        ...
        examName: window.EXAM_NAME || 'UCAT Decision Making Mock 2',
        questions: QUESTIONS,
...
function renderResults(result, saveRes) {
  ...
  if (isSJT) {
    ...
    const secName = window.EXAM_NAME || 'UCAT Decision Making Mock 2';
  ...
  wrap.innerHTML = `
    <div class="res-hero">
      <div style="font-size:0.8rem;opacity:.55;text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px">${window.EXAM_NAME || 'UCAT Decision Making Mock 2'}</div>
      ${scoreHeroHTML}
